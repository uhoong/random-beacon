...
void DrgCore::deliver_chunk_evil()
{
    ...
    for (int i = config.nreplicas / 2; i < config.nreplicas; i++) // 给后面一半人发
    {
...
{
    if (evilNodes_notForward.find(id) != evilNodes_notForward.end())
    {
        do_broadcast_sharechunk_evil(shareChunk);
    }
    else
    {
        ...
        if (sharechunk_matrix[shareChunk.replicaID].find(shareChunk.idx) == sharechunk_matrix[shareChunk.replicaID].end())
        {
            ...
            {
                sharechunk_matrix[shareChunk.replicaID][shareChunk.idx] = shareChunk.chunk;
                do_broadcast_sharechunk_random(shareChunk);
                qsize++;
            }
        }
...
void DrgCore::on_init(std::unordered_set<ReplicaID> &replicas_notSharing, std::unordered_set<ReplicaID> &replicas_notForward,int probility)
{
    ...
    for (auto it = replicas_notForward.begin(); it != replicas_notForward.end(); it++)
    {
        evilNodes_notForward.insert(*it);
    }
}